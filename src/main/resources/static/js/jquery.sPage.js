...
; (function ($, window, document, undefined) {
    ...
    $.extend(Plugin.prototype, {
        ...
        viewHtml: function () {
            ...
            var pageTotal = 0;
            if (settings.total > 0) {
                pageTotal = Math.ceil(settings.total / settings.pageSize);
            } else {
                if (settings.noData === 'true') {
                    pageTotal = 1;
                    settings.page = 1;
                    settings.total = 0;
                } else {
            ...
            if (pageTotal <= 6) {
                ...
                if (settings.page < 5) {
                    ...
                    this.pageList.push('<span data-page="none">...</span><span data-page=' + pageTotal + '>' + pageTotal + '</span>');
                } else if (settings.page > pageTotal - 4) {
            ...
            if (settings.showPN === 'true') {
                settings.page == pageTotal ? this.pageList.push('<span class="span-disabled" data-page="next">' + settings.nextPage + '</span>') : this.pageList.push('<span data-page="next">' + settings.nextPage + '</span>');
            }
            pageArr.push(this.pageList.join(''));
            pageArr.push('</div>');
            if (settings.showSkip === 'true') {
                pageArr.push('<div class="spage-skip">跳至&nbsp;&nbsp;<input type="text" value="' + settings.page + '"/>&nbsp;页&nbsp;&nbsp;<span data-page="go">跳转</span></div>');
            }
        ...
        clickBtn: function () {
            ...
            this.element.on("click", "span", function () {
                ...
                switch (pageText) {
                    case "prev":
                        ...
                        break;
                    case "next":
                        settings.page = Number(settings.page) + 1 <= pageTotal ? Number(settings.page) + 1 : pageTotal;
                        pageText = settings.page;
                        break;
                    case "none":
                        return;
            ...
            this.element.on("keyup", "input", function (event) {
                if (event.keyCode == 13) {
                    var p = parseInt(ele.find("input").val());
                    if (/^[0-9]*$/.test(p) && p >= 1 && p <= pageTotal && p != that.pageNum) {
                        settings.page = p;
                        that.pageNum = p;
                        that.viewHtml();
                        settings.backFun(p);
